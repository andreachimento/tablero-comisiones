...
// ----------------------------------------------------------------------------
// Quien cuenta como "profesor" de una comision
// ----------------------------------------------------------------------------
...
//
// El criterio ahora es por descarte y no por lista blanca: se apartan los
// roles que explicitamente NO son el titular (TUTOR, SUPLENTE, y cualquier
// asignacion marcada como reemplazo/suplencia) y todo el resto -incluido el
// rol vacio- cuenta como profesor. Asi, si en el futuro aparece un rol nuevo
// que hoy no conocemos, la persona igual se ve en el tablero en vez de
// desaparecer en silencio.
function esTutor(a) {
  return a && a.cohortRole === 'TUTOR';
}

function esSuplente(a) {
  return !!(a && (a.cohortRole === 'SUPLENTE' || a.isReplacement));
}

function esProfesor(a) {
  return !!a && !esTutor(a) && !esSuplente(a);
}
