...
function computeColorReason(estadoOverlay, overlapCheck, ratingPromedio, habilitado, esNuevo) {
  ...
  if (esNuevo) {
    return { color: 'violeta', reason: 'Disponible · todavía no tiene asignada su primera comisión' };
  }
  if (ratingPromedio != null && ratingPromedio <= 4.5) {
    return { color: 'rojo', reason: 'No se puede sumar · rating ' + ratingPromedio.toFixed(1) + ' (≤ 4.5)' };
  }
  if (ratingPromedio != null && ratingPromedio <= 4.7) {
    return { color: 'naranja', reason: 'Puede sumarse, pero rating ' + ratingPromedio.toFixed(1) + ' (4.5 a 4.7)' };
  }
  return {
    color: 'verde',
