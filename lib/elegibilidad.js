...
//
// Reglas (confirmadas por Andrea, actualizadas set. 2026 - se saco la
// categorizacion por comentario y se paso ese criterio al Estado del
// perfil, que ahora tiene 5 valores en vez de 4):
//   - ROJO: estado despido / en consideracion futura / renuncio, o
//     superposicion COMPLETA de horario con otra comision propia vigente
//     (asignada o en curso), o rating <= 4.5.
//   - NARANJA: estado "asignable con alerta" (fuerza naranja pase lo que
//     pase con horario/rating), o superposicion PARCIAL (algunos dias, no
//     todos), o sin superposicion pero rating > 4.5 y <= 4.7.
//   - VERDE: sin superposicion y rating > 4.7, o sin superposicion y
//     todavia sin ningun rating cargado (pero ya con al menos una comision
//     asignada alguna vez - ver VIOLETA).
//   - VIOLETA: sin superposicion, estado normal, pero la persona todavia
//     nunca fue asignada a NINGUNA comision (recien ingreso) - reemplaza al
//     calculo por rating hasta que se la asigne por primera vez.
// ============================================================================
...
// ----------------------------------------------------------------------------
// Con que rol hay que mostrar una asignacion
// ----------------------------------------------------------------------------
// El back office permite que una asignacion quede guardada SIN cohortRole: en
// la pantalla de la comision la persona se ve normal, bajo "Staff Titular",
// pero la API devuelve el campo cohortRole vacio. Los tableros filtraban con
// (cohortRole === 'INSTRUCTOR' || cohortRole === 'PROFESOR'), asi que esa
// gente no entraba en ninguna de las dos listas y la comision figuraba "Sin
// asignar" aunque tuviera staff cargado (caso detectado: comision 90945, con
// Brenda Dujovich asignada en el BO y vacia en el tablero; le pasaba lo mismo
// a sus comisiones 92765, 94710 y 95010).
//
// Cuando el rol no viene, se deduce del mail de la cuenta: en Coderhouse cada
// persona usa una cuenta distinta por rol, con un +tag antes de la arroba
// (fulano+profesor@x.com para dar clases, fulano+tutora@x.com para tutorear).
// Es el mismo criterio con el que el resto del tablero agrupa las cuentas de
// una persona (ver personKey en lib/overlay.js), asi que un tutor sin rol
// cargado se sigue contando como tutor y no se cuela en la columna Profesor.
//
// Los tags no estan normalizados: sobre las 692 cuentas con asignaciones en
// el back office conviven profesor (262), tutor (194), profesora (119),
// tutora (98), prof (3), professor, profecc, profesorcc, profesor1000,
// tutoral... Por eso se compara por prefijo (^prof / ^tutor) y no por
// igualdad: asi entran todas las variantes de escritura sin tener que
// mantener una lista.
//
// Quedan 11 cuentas (1,6%) cuyo tag no dice nada del rol - 4 sin ningun tag,
// y el resto cuentas internas o de prueba (+staff, +ticher, +juandemo,
// +pruebita00, +testprofesor). Para esas devolvemos null y quien llama
// decide; hoy caen en profesor. Si alguna de ellas llegara a recibir una
// asignacion de tutor SIN rol cargado, se veria en la columna Profesor.
function rolPorEmail(email) {
  const local = String(email || '').toLowerCase().trim().split('@')[0];
  const plus = local.indexOf('+');
  if (plus === -1) return null;
  const tag = local.slice(plus + 1).normalize('NFD').replace(/[̀-ͯ]/g, '');
  if (/^tutor/.test(tag)) return 'TUTOR';
  if (/^prof/.test(tag)) return 'PROFESOR';
  return null;
}

// Devuelve siempre uno de: 'PROFESOR' | 'TUTOR' | 'SUPLENTE'.
// Orden de prioridad:
//   1. la asignacion marcada como suplencia/reemplazo manda sobre todo
//   2. el cohortRole que mando el back office, si vino cargado
//   3. el +tag del mail de la cuenta
//   4. profesor, como ultimo recurso (las 5 cuentas sin tag, y cualquier rol
//      nuevo que aparezca mas adelante) - preferimos mostrar a la persona en
//      la columna principal antes que perderla del tablero.
function clasificarAsignacion(a, email) {
  if (a && a.isReplacement) return 'SUPLENTE';
  const rol = (a && a.cohortRole) || rolPorEmail(email) || '';
...
//
// esNuevo (opcional, default false): la persona todavia nunca fue asignada
// a NINGUNA comision (ni siquiera una ya finalizada). Mientras sea asi, sus
// postulaciones se marcan VIOLETA en vez de calcularse por rating (todavia
// no tiene uno real) - asi Andrea distingue "recien esta arrancando" de "ya
// tiene historial pero con mal rating". En cuanto se la asigna a su primera
// comision (esNuevo pasa a false), el resto de sus postulaciones vuelve a
// tomar el color que le corresponda por rating, como al resto del staff.
//
// Nota (set. 2026): antes existia ademas una categorizacion manual POR
// COMENTARIO ("No asignable" / "Asignable con alerta" / "Asignable") que se
// sumaba a estas reglas. Andrea pidio sacarla del todo: ese criterio ahora
// se carga directamente en el Estado del perfil (mas abajo), que sumo el
// valor 'alerta' para cubrir el mismo caso a nivel de toda la persona, no
// comentario por comentario.
function computeColorReason(estadoOverlay, overlapCheck, ratingPromedio, habilitado, esNuevo) {
  if (habilitado === false) {
    return { color: 'gris', reason: 'Se postuló pero no está habilitado para dictar este curso · revisar antes de aprobar' };
  }
  // 'desaprobado' se deja como alias del valor viejo (antes de set. 2026,
  // cuando este mismo estado se llamaba asi) para que ningun perfil ya
  // guardado con ese valor deje de mostrarse en rojo - el selector del
  // perfil ya solo ofrece 'despido' de aca en mas.
  if (estadoOverlay === 'despido' || estadoOverlay === 'desaprobado') {
    return { color: 'rojo', reason: 'No se puede sumar · despido' };
  }
  if (estadoOverlay === 'futuro') {
    return { color: 'rojo', reason: 'No se puede sumar · en consideración futura' };
  }
  if (estadoOverlay === 'renuncia') {
    return { color: 'rojo', reason: 'No se puede sumar · renunció' };
  }
  if (estadoOverlay === 'alerta') {
    return { color: 'naranja', reason: 'Puede sumarse, pero con alerta en su estado · a confirmar' };
  }
  ...
  if (overlapCheck.type === 'completa') {
  ...
  } else if (overlapCheck.type === 'parcial') {
    const ref = overlapCheck.refs[0];
    base = {
      color: 'naranja',
    ...
    base = { color: 'violeta', reason: 'Disponible · todavía no tiene asignada su primera comisión' };
  } else if (ratingPromedio != null && ratingPromedio <= 4.5) {
    base = { color: 'rojo', reason: 'No se puede sumar · rating ' + ratingPromedio.toFixed(1) + ' (≤ 4.5)' };
  } else if (ratingPromedio != null && ratingPromedio <= 4.7) {
    base = { color: 'naranja', reason: 'Puede sumarse, pero rating ' + ratingPromedio.toFixed(1) + ' (4.5 a 4.7)' };
  } else {
    base = {
      color: 'verde',
...
module.exports = {
  ...
  rolPorEmail, clasificarAsignacion,
