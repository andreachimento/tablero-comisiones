...
//   - ROJO: desaprobado / en consideracion futura / renuncio, o
...
//     superposicion pero rating > 4.5 y <= 4.7.
...
//
// categoriaComentario (opcional, default null - "sin categorizar"): la
// subcategoria MAS restrictiva entre todos los comentarios de la persona
// (ver worstCategoria() en lib/overlay.js), cargada a mano por el equipo,
// nunca asignada sola. Se suma como un parametro MAS a las reglas de
// arriba, sin reemplazarlas ("stackea"):
//   - 'no_asignable'     -> fuerza ROJO, pase lo que pase con horario/rating
//     (misma jerarquia que un estado desaprobado/renuncia).
//   - 'asignable_alerta' -> pone un TECHO en NARANJA: si por horario/rating
//     el resultado ya era rojo o naranja, no cambia nada; si iba a dar
//     verde o violeta, se baja a naranja con alerta.
//   - 'asignable' / null -> no restringe nada extra (no fuerza verde: si el
//     horario o el rating ya decian rojo/naranja, eso se respeta igual).
function computeColorReason(estadoOverlay, overlapCheck, ratingPromedio, habilitado, esNuevo, categoriaComentario) {
  if (habilitado === false) {
    return { color: 'gris', reason: 'Se postuló pero no está habilitado para dictar este curso · revisar antes de aprobar' };
  }
  if (categoriaComentario === 'no_asignable') {
    return { color: 'rojo', reason: 'No se puede sumar · categorizado como "No asignable" en sus comentarios' };
  }
  ...
  if (categoriaComentario === 'asignable_alerta' && (base.color === 'verde' || base.color === 'violeta')) {
    return { color: 'naranja', reason: base.reason + ' · con alerta en sus comentarios (a confirmar)' };
  }
  return base;
}
