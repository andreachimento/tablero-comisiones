...
// Excepciones: personas puntuales (sin +profesor/+tutor todavia, ej. en
// proceso de seleccion) habilitadas a mano para rendir UN curso+rol
// especifico sin el filtro normal de mail. Se consume sola (se borra) apenas
// la persona rinde esa evaluacion una vez, la haya aprobado o no.
const EXCEPCIONES_KEY = 'staff:cert-excepciones';
...
// ---------------------------------------------------------------------------
// Excepciones (candidatos sin +profesor/+tutor habilitados a mano)
// ---------------------------------------------------------------------------

async function getAllExcepciones() {
  const redis = getRedis();
  const raw = await redis.hgetall(EXCEPCIONES_KEY);
  const out = [];
  if (raw) {
    Object.keys(raw).forEach(id => {
      const parsed = parseValue(raw[id]);
...
async function addExcepcion({ nombre, email, curso, rol, agregadaPor }) {
  const redis = getRedis();
  const excep = {
    id: genId(),
  ...
  await redis.hset(EXCEPCIONES_KEY, { [excep.id]: JSON.stringify(excep) });
  return excep;
}

async function quitarExcepcion(id) {
  const redis = getRedis();
  await redis.hdel(EXCEPCIONES_KEY, id);
}
...
module.exports = {
  ...
  getAllExcepciones, addExcepcion, quitarExcepcion,
};
