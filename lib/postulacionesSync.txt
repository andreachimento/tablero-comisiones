...
// el perfil de cada persona y en cada comision. La corre una vez por hora el
// cron de Vercel (ver api/postulaciones-sync.js + la seccion "crons" de
// vercel.json).
//
// Que hace, paso a paso:
//   1. Lee TODAS las filas de la planilla (via lib/googleSheet.js).
...
async function syncPostulacionesFromSheet() {
  const sheetEnv = getSheetEnv();
  const env = getBackofficeEnv();

  const rows = await getSheetValues(sheetEnv.spreadsheetId, sheetEnv.gid);
...
// Se llama desde api/dashboard-data.js (la pestaña que mas se abre) en vez
// de un cron por hora: los cron jobs de Vercel en el plan gratuito ("Hobby")
// solo pueden correr una vez por dia, un cron cada una hora rompe todo el
// despliegue. En cambio, chequear "hace cuanto fue la ultima sincronizada de
// verdad" cada vez que alguien abre el tablero, y volver a sincronizar solo
// si paso mas de una hora, logra el mismo resultado practico (las
// respuestas nuevas aparecen dentro de la hora) sin depender de eso. Como
// respaldo, ademas hay un cron DIARIO (permitido en el plan gratuito, ver
// vercel.json) que sincroniza igual aunque nadie haya abierto el tablero en
// todo el dia.
...
module.exports = { syncPostulacionesFromSheet, ensureFreshSync };
