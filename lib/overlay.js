...
// ----------------------------------------------------------------------------
// Subcategoria manual de comentarios (pedido de Andrea, migracion Notion CRM
// Relaciones Laborales, set. 2026): cada comentario puede tener una
// categoria que el equipo carga A MANO (nunca se asigna sola) para
// clasificar que tan asignable es la persona segun ese comentario puntual:
//   - 'no_asignable'      -> Rojo. La persona no se puede sumar a nada.
//   - 'asignable_alerta'  -> Naranja, con status aprobado. Se puede sumar
//                            pero con alerta / a confirmar antes.
//   - 'asignable'         -> Verde, con status aprobado. Sin objeciones.
//   - null / undefined    -> Sin categorizar (default para TODO comentario
//                            nuevo, tanto los que carga el equipo a mano
//                            como los importados automaticamente de Notion).
// "worst wins": si una persona tiene varios comentarios categorizados, el
// que aplica es el mas restrictivo (no_asignable > asignable_alerta >
// asignable > sin categorizar) - ver worstCategoria() y su uso en
// lib/elegibilidad.js (computeColorReason), que lo suma como un parametro
// MAS a las reglas existentes (estado, superposicion, rating), sin
// reemplazarlas.
// ----------------------------------------------------------------------------
const CATEGORIA_COMENTARIO_VALUES = ['no_asignable', 'asignable_alerta', 'asignable'];
const CATEGORIA_RANK = { no_asignable: 3, asignable_alerta: 2, asignable: 1 };

function worstCategoria(comentarios) {
  let worst = null;
  let worstRank = 0;
  (comentarios || []).forEach(c => {
    const cat = c && c.categoria;
    const rank = CATEGORIA_RANK[cat] || 0;
    if (rank > worstRank) { worstRank = rank; worst = cat; }
  });
  return worst; // null = nadie categorizado, o solo categorizados como "asignable"-neutro sin nada peor
}
...
// Guarda (o actualiza) en un solo paso TODAS las postulaciones que vienen de
// la planilla de Google Sheets del sitio publico (ver lib/postulacionesSync.js
// y api/postulaciones-sync.js). Se llama una vez por hora, siempre con la
// planilla entera ya "traducida" a este formato.
//
// Es "upsert" por id (mismo id = misma fila de la planilla = misma persona +
// misma comision, sin importar si se postulo una sola vez o varias): si ya
// existia, se actualizan sus datos (nombre, curso, etc, por si cambiaron) PERO
// se conserva el estado (pendiente/aprobada/rechazada) que ya tuviera, para
// no pisar una decision que Andrea ya tomo desde el tablero. Si es nueva,
// arranca en "pendiente". Las postulaciones que ya estaban guardadas y no
// vienen en esta tanda (ej. se borro la fila en la planilla) NO se tocan ni
// se borran, para no perder nada por un problema pasajero leyendo la
// planilla.
async function upsertPostulacionesFromSheet(nuevas) {
  const redis = getRedis();
  const existentesRaw = await redis.lrange(POSTULACIONES_KEY, 0, -1);
  const existentes = (existentesRaw || []).map(v => parseValue(v)).filter(Boolean);
  const porId = new Map();
  existentes.forEach(p => { if (p && p.id) porId.set(p.id, p); });
  ...
  (nuevas || []).forEach(n => {
    ...
    if (previa) {
      porId.set(n.id, { ...previa, ...n, estado: previa.estado || 'pendiente' });
    } else {
      porId.set(n.id, { ...n, estado: 'pendiente' });
      nuevosCount++;
    }
  });

  const finalList = Array.from(porId.values());
  await redis.del(POSTULACIONES_KEY);
  const BATCH = 200;
  for (let i = 0; i < finalList.length; i += BATCH) {
    const chunk = finalList.slice(i, i + BATCH).map(v => JSON.stringify(v));
    if (chunk.length) await redis.rpush(POSTULACIONES_KEY, ...chunk);
  }
  return { total: finalList.length, nuevas: nuevosCount, actualizadas: (nuevas || []).length - nuevosCount };
}

// Carga en un solo paso la disponibilidad (dias/franjas) que respondieron
// profesores y tutores en la encuesta de Google Sheets, para todos a la vez
// - ver lib/disponibilidadSync.js. Se agrupa por personKey (igual que
// admin-seed-staff.js), asi que si la misma persona contesto con dos mails
// distintos (ej. uno +profesor y otro +tutor) queda unificado en un solo
// perfil. Si la persona ya tenia un perfil cargado, se le pisa SOLO el
// campo disponibilidad (con lo que contesto en la encuesta, que se toma
// como la fuente de verdad) - el resto del perfil (comentarios, cursos
// habilitados, estado, ratings) no se toca.
async function setDisponibilidadBulk(nuevas) {
  const redis = getRedis();
  const existing = await getAllOverlays();

  const porPersona = {};
  (nuevas || []).forEach(entry => {
    const rawEmail = normalize(entry.email);
    if (!rawEmail) return;
  ...
  Object.keys(porPersona).forEach(key => {
    // Si llegaron varias respuestas para la misma persona (dos mails, o
    // volvio a contestar la encuesta), nos quedamos con la mas reciente
    // segun 'fecha' (el "Submitted at" de la encuesta, que ordena bien
    // como texto: "2026-08-21 20:31:44").
    const entradas = porPersona[key].slice().sort((a, b) => String(b.fecha || '').localeCompare(String(a.fecha || '')));
    const masReciente = entradas[0];

    let overlay = existing[key];
    if (!overlay) { overlay = defaultOverlay(); nuevosPerfiles++; }
    overlay.disponibilidad = {
      dias: Array.isArray(masReciente.dias) ? masReciente.dias : [],
      franjas: Array.isArray(masReciente.franjas) ? masReciente.franjas : [],
    };
    if (!overlay.nombre && masReciente.nombre) overlay.nombre = masReciente.nombre;
  ...
  return { personasActualizadas: entries.length, perfilesNuevos: nuevosPerfiles };
}
...
module.exports = {
  ...
  addPostulacion, getAllPostulaciones, setPostulacionEstado, upsertPostulacionesFromSheet, POSTULACIONES_KEY,
  setDisponibilidadBulk,
  CATEGORIA_COMENTARIO_VALUES, worstCategoria,
};
